...
window.zoomEarth = function(zoomAmount) {
  ...
  if (typeof window.onEarthZoom === 'function') {
    ...
    goHash({'orthographic': localObject.earth.orthographic});
  }
};
function delay(time) {
  ...
  waitForElm('#slider-pips').then(function() {
    ...
    pipsSlider.noUiSlider.on('set', function(values) {
      ...
      if (options && options.onSliderSet) {
        ...
        var baseDate = (currentHash && currentHash.when && currentHash.when.match(/^(\d{4})\/(\d{2})\/(\d{2})\//));
        var now = baseDate
          ? new Date(Date.UTC(Number(baseDate[1]), Number(baseDate[2]) - 1, Number(baseDate[3])))
          : new Date();
        if (hour >= 24) now.setUTCDate(now.getUTCDate() + 1);
        ...
        if (ortho) loadIframe('mainframe', 'https://earth.nullschool.net/#' + when + '/wind/surface/level/orthographic=' + ortho);
        goHash({'when': when});
      }
    });
    ...
    if (options && options.onSliderReady) {
      ...
      var h = getHash();
      if (h && h.when) {
        var whenMatch = h.when.match(/\/(\d{2})00Z$/);
        if (whenMatch) pipsSlider.noUiSlider.set(Number(whenMatch[1]));
      }
    }
    ...
    document.addEventListener('hashChangeEvent', function() {
      ...
      if (!hash.when || hash.when === prior.when) return;
      var whenMatch = hash.when.match(/\/(\d{2})00Z$/);
      if (!whenMatch) return;
      var hour = Number(whenMatch[1]);
      var sliderHour = hour === 0 ? 24 : hour;
      if (!(options && options.onSliderSet)) {
        var ortho = (localObject && localObject.earth && localObject.earth.orthographic) || (options && options.defaultLatLonZoom) || '';
        if (ortho) loadIframe('mainframe', 'https://earth.nullschool.net/#' + hash.when + '/wind/surface/level/orthographic=' + ortho);
      }
...
function showEarthEndMessage() {
  ...
  msg.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,.65);color:#fff;padding:8px 18px;border-radius:6px;font-size:15px;pointer-events:none;z-index:200;white-space:nowrap;';
...
window.startEarthSliderPlayback = function() {
  ...
  if (Number(earthSliderRef.noUiSlider.get()) >= 24) earthSliderRef.noUiSlider.set(3);
  function step() {
    var next = Number(earthSliderRef.noUiSlider.get()) + earthSliderStep;
    if (next > 24) { window.stopEarthSliderPlayback(); showEarthEndMessage(); return; }
