...
A very rough estimate for local industry employee counts and payroll can be generated by averaging from other counties in the state, however some states have 80+ industries out of 388 without any employee and payroll data.

We'll be using attributes in the impact data used by the [USEEIO widgets](../../../../io/charts/) to make more accurate predictions based on levels in counties with similar industries and attributes, including population density and area of country using the lat/lon centroid values for each county. 
