...
To prepare data for our [industry impact page](https://model.earth/localsite/info/), we are using [GitHub Actions](/community/projects/#github-actions) and two processes that estimate gaps in BLS data: Ecker Linear Objective Functions and Taylor Machine Learning.  

We process NAICS industry data into [state files with county employment levels](https://github.com/modelearth/community-data/tree/master/us/state) which are used client-side to filter [US EPA Input-Output Charts](../../../io/charts/) loaded from [static JSON impact data](https://github.com/modelearth/io/tree/main/build/api).  
